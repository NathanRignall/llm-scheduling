    ...
    def evaluate(self, islands, trace_pdf, print_debug=True):
        ...
        for island_id, island in islands.items():
            if island.role == "prefill":
                # for each assignment in the island
                for (bin, value) in island.assignments:
        ...
        for sequence_length, result in results.items():
            # sum the throughput for each island
            total_throughput = 0
            for island_id, throughput in result["prefill_throughputs"].items():
                total_throughput += throughput["weighted_request_per_second"]
